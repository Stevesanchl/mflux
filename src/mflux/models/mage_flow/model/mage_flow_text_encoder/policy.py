...
class FilterVerdict:
    ...
    def banner(self) -> str:
        if not self.violates:
            return ""
        category = ", ".join(CATEGORY_DISPLAY.get(item, item) for item in self.categories) or "policy violation"
        return f"🚫 **Content Filter:** Blocked — `{category}` · {self.reason}"


class MageFlowContentPolicy:
    @classmethod
    def screen_text(
        cls,
        *,
        ...
        try:
            ...
            input_ids = mx.array(np.asarray(tokens["input_ids"]), dtype=mx.int32)
            attention_mask = mx.array(np.asarray(tokens["attention_mask"]), dtype=mx.int32)
            generated_ids = text_encoder.generate_greedy(
                input_ids,
                attention_mask=attention_mask,
                max_new_tokens=max_new_tokens,
                eos_token_id=raw_tokenizer.eos_token_id,
            )
            generated = raw_tokenizer.decode(
                np.asarray(generated_ids[0]).tolist(),
                skip_special_tokens=True,
            ).strip()
            return cls._parse_verdict(generated)
        except Exception as error:  # noqa: BLE001
            return FilterVerdict(
                True,
                ["policy"],
                f"filter error (blocked): {type(error).__name__}: {error}",
                "",
            )
    ...
    def screen_edit(
        ...
        *,
        ...
        try:
            ...
            processor = MageFlowQwen3VLProcessor(
                raw_tokenizer,
                max_long_edge=None,
            )
            inputs = processor(
            ...
            return FilterVerdict(
                True,
                ["policy"],
                f"edit filter error (blocked): {type(error).__name__}: {error}",
                "",
            )

    @staticmethod
    def _raw_tokenizer(tokenizer: Any) -> Any:
